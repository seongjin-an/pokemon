...
const Evolution: React.FC<Props> = ({
    ...
    const [evolutionChain, setEvolutionChain] = useState<{from: {name: string; url: string}, to: {name: string; url: string}, level: number}[]>([]);
                        ...
                        <List>
                            {
                                evolutionChain.map(({from, to, level}, idx) => (
                                    <EvolutionStage from={from} to={to} level={level}/>
                                ))
                            }
                        </List>
                    ) : (
